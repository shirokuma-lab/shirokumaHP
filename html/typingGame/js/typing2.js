'use strict';
    const textList = [
      'print','document','include','stdio.h','String','scan','public','static','void','main','stylesheet','input','output','getElementById','textContent',
      'createElement','setAttribute','function','appendChild','script','length','import','python','android','header','footer','content','position','decoration',
      'padding','margin','section','onclick'
    ];
    const syogo = ["おさるさん", "パソコン初心者", "ある程度キーボード打てる奴", "パソコン中級者", "未来を支えるエンジニア予備軍", "パソコン上級者", "タイピングマスター", "IT魔神", "かみさま"]
    let count1 = 0;
    let count2 = 0;
    let img;
    let toi = textList[Math.floor(Math.random() * textList.length)];
    let sec = 90;
    let point = 0;
    ...
    document.getElementById('start').onclick = function(){
      ...
      document.getElementById('mondai2').textContent = toi;
      ...
      setInterval(function(){
        ...
        if (sec == 0) {
          ...
          againGame.setAttribute("href", "hutu.html");
      ...
      function init() {
        ...
        document.getElementById('mondai2').textContent = textList[rnd];
        inputForm.kaito.value = '';
        inputForm.kaito.focus();
        toi = document.getElementById('mondai2').textContent;
